...
function generateXMLConditions() {
  ...
  for (const elem of organizedData) {
    ...
    if (cheker.length <= 1) {
      ...
      concate += ")";
      if (cheker[0] != "site") {
        concate += ` AND (us.fnd0getGroupName()=&quot;${HandleSpecialaCharInGroupName(cheker[0])}&quot;)`;
      }
      ...
      for (let i = 0; i < elem.objects.length; i++) {
        ...
        if (i == 0) {
          if (elem.objects[i].group == "site") {
            ...
            concate += `(wo.object_type=&quot;${elem.objects[i].object_type}&quot; AND us.fnd0getGroupName()=&quot;${HandleSpecialaCharInGroupName(elem.objects[i].group)}&quot;)`;
          }
        } else {
          if (elem.objects[i].group == "site") {
            concate += ` OR (wo.object_type=&quot;${elem.objects[i].object_type}&quot;)`;
          } else {
            concate += ` OR (wo.object_type=&quot;${elem.objects[i].object_type}&quot; AND us.fnd0getGroupName()=&quot;${HandleSpecialaCharInGroupName(elem.objects[i].group)}&quot;)`;
          }
        }
      }
...
function HandleSpecialaCharInGroupName(groupname){
